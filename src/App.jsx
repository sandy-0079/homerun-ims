...
function OverviewTab({ invoiceData, results, priceData, params, invoiceDateRange,
  ...
  const getInv = useCallback((r, field) => {
    ...
    // "All" = full network (DS01–DS05 + DC) so table rollups tie out to the KPI cards,
    // which always include DC (see kpis useMemo above).
    return DS_LIST.reduce((s, ds) => s + (r.stores[ds]?.[field] || 0), 0) + (r.dc?.[field] || 0);
  }, [store]);
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8, flexWrap: "wrap" }}>
        ...
        <select value={store} onChange={e => setStore(e.target.value)}
          ...
          <option value="All">All Locations (incl. DC)</option>
...
const zeroSaleLists = useMemo(
  ...
  const ADMIN_TABS=[["overview","Overview"],["skuDetail","SKU Detail"],["baskets","Baskets"],["plywood","Plywood"],["stockHealth","Stock Health"],["output","Tool Output Download"],["upload","Upload Data"],["logic","Logic Tweaker"],["overrides","Manual Overrides"]];
  const PUBLIC_TABS=[["overview","Overview"],["skuDetail","SKU Detail"],["baskets","Baskets"],["plywood","Plywood"],["stockHealth","Stock Health"],["output","Tool Output Download"]];
